...
import {
  ...
  Bell, Users, MapPin, LogOut, Menu, X, Calendar
} from "lucide-react";
...
const AdminDashboard = ({ onLogout, user }: Props) => {
    ...
    <div className="min-h-screen flex bg-section-alt">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-primary text-primary-foreground flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <div className="p-3 border-t border-sidebar-border">
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-primary-foreground/80 hover:bg-sidebar-accent/50 transition-colors"
          >
